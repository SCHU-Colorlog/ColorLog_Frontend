...
const Colorlog = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8080/api/api/user/get_result`, {
        params: { userId }
    })
        .then(response => {
...
useEffect(() => {
    async function fetchData() {
        try {
            const response = await axios.get('http://localhost:8080/api/api/photogroup/get_photogroup', {
                params: { userId }
            });
